...
function ServedMeals({servedMeals=[],email}){
    const {loading} = useAuth();
    const {getStudentMeals} = useMealCURD();
    const {data, isLoading, isError, error} = useQuery({
        queryKey:["studentMeals", servedMeals],
        queryFn:()=>getStudentMeals(servedMeals),
  ...
  return (
    <>
        <HeadingTitle headingData={{heading:"Served Meals"}}/>
    <section>
    <section>
    <section className="w-full mx-auto">
      <div className="overflow-x-auto">
  {
    data?.length ? <table className="table text-center text-gray-400">
    ...
    <tbody>
      {/* row 1 */}
      {
        data?.map((item, index)=><TableRow key={item._id} idx={index} item={item}/>)
      }
    </tbody>
  </table> : 
  <h1 className="text-center text-2xl my-10 font-bold text-orange-600">Nothing Served Yet</h1>
  }
</div>
      </section>
    </section>
      ...
      <tr>
        <td className="w-[20px] text-lg font-heading italic">
          ...
          <td className="">
           ...
           <button onClick={()=>(console.log("object"))} className=" bg-red-800  z-20 px-4 py-1 rounded-e-full rounded-s-full text-white border-[1px] border-red-800 font-semibold h-full">
              Cancel
          </button>  
